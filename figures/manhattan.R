...
manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
                      ...
                      suggestiveline=-log10(1e-5), genomewideline=-log10(1e-9), 
  ...
  if (nchr==1) {
    with(d, points(pos, logp, pch=20, col=col[1], cex = 2, ...)) # changed
  } else {
    # if multiple chromosomes, need to alternate colors and increase the color index (icol) each chr.
    icol=1
    for (i in unique(d$index)) {
      #with(d[d$index==unique(d$index)[i], ], points(pos, logp, col=col[icol], pch=20, ...))
      points(d[d$index==i,"pos"], d[d$index==i,"logp"], col=col[icol], pch=20, cex = 2, ...) # changed
      icol=icol+1
    }
  }
  ...
  if (!is.null(highlight)) {
    ...
    with(d.highlight, points(pos, logp, col="green3", pch=20, cex = 2, ...)) # changed
  }
    ...
    if (annotateTop == FALSE) {
      if (logp) {
        with(subset(d, P <= annotatePval), 
             textxy(pos, -log10(P), offset = 0.625, labs = topHits$SNP, cex = 1.4), ...) # changed
      } else
        with(subset(d, P >= annotatePval), 
             textxy(pos, P, offset = 0.625, labs = topHits$SNP, cex = 1.4), ...) # changed
    }
    else {
